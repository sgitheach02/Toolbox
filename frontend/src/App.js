...
// Service API
// Service API CORRIGÉ avec nouveaux endpoints
// Service API CORRIGÉ avec nouveaux endpoints
const apiService = {
  ...
  async startHydraAttack(target, service, username, wordlist, bruteforce_usernames, attack_mode) {
    return this.request('/api/scan/hydra', {
      method: 'POST',
      body: JSON.stringify({ 
        target, 
        service, 
 ...
 async startMetasploitExploit(exploit, target, payload, lhost) {
  return this.request('/api/scan/metasploit', {
    method: 'POST',
  ...
  async stopTcpdumpCapture(taskId) {
    return this.request(`/api/scan/tcpdump/${taskId}/stop`, {
      method: 'POST'
    });
  },
  ...
  // Téléchargement direct d'URL CORRIGÉ
  getDownloadUrl(taskId, fileType = 'report') {
    return `${API_BASE_URL}/api/scan/download/${taskId}${fileType === 'pcap' ? '/pcap' : ''}`;
  },

  // Analyse PCAP corrigée avec vrai endpoint
...
const NmapTab = () => {
 ...
 useEffect(() => {
  ...
  const savedActiveTask = persistenceService.load('nmap_active_task', null); // ← NOUVEAU
  
  setResults(savedResults);
  if (savedForm.target) setTarget(savedForm.target);
  if (savedForm.scanType) setScanType(savedForm.scanType);
  
  // ← NOUVEAU : RESTAURER LA TÂCHE ACTIVE
  if (savedActiveTask && savedActiveTask.taskId) {
    console.log(`🔄 Nmap: Restauration tâche active ${savedActiveTask.taskId}`);
    setCurrentTaskId(savedActiveTask.taskId);
    setTarget(savedActiveTask.target || target);
    setScanType(savedActiveTask.scanType || scanType);
  }
  ...
  useEffect(() => {
  if (currentTaskId) {
    persistenceService.save('nmap_active_task', {
    ...
    persistenceService.remove('nmap_active_task');
  }
...
const MetasploitTab = () => {
  ...
  const startExploit = async () => {
    ...
    setError('');
    try {
      const response = await apiService.startMetasploitExploit(exploit, target, payload, lhost);
      setCurrentTaskId(response.task_id);
    } catch (error) {
      setError(error.message);
    }
 ...
 const openSessionInteraction = (session) => {
  // Ajouter les infos manquantes à la session
  const enhancedSession = {
    ...
    opened_at: session.opened_at || new Date().toLocaleTimeString()
  };
  console.log('🔗 Ouverture interaction session:', enhancedSession);
  setActiveSession(enhancedSession);
};
...
const HydraTab = () => {
  ...
  const startAttack = async () => {
    ...
    setError('');
    try {
     const response = await apiService.startHydraAttack(
  target, 
  service, 
  bruteforceMode === 'single' ? username : null,
  (attackMode === 'wordlist' || attackMode === 'combo') ? `/usr/share/wordlists/${wordlist}.txt` : null,
...
// ================================
// DOWNLOAD MANAGER ET REPORTING - AMÉLIORÉ POUR TOUS LES SCANS
// ================================
const DownloadManager = ({ taskId, taskData, taskStatus, tool }) => {
  ...
  const downloadPcapFile = () => {
    if (!taskId) return;
    const url = `${API_BASE}/api/scan/download/${taskId}/pcap`;
    window.open(url, '_blank');
  };
  ...
  const generateHtmlReport = async () => {
    ...
    try {
      ...
      // Auto-téléchargement
      const link = document.createElement('a');
      link.href = `${API_BASE}${response.report_url}`;
  ...
  const exportCredentials = () => {
    if (!taskData?.results?.credentials_found) return;
    
    const credentials = taskData.results.credentials_found;
    const csvContent = "data:text/csv;charset=utf-8," 
      + "Username,Password,Target,Service,Timestamp\n"
      + credentials.map(cred => {
          const [username, password] = cred.split(':');
          return `${username},${password},${taskData.target || 'N/A'},${taskData.service || 'N/A'},${taskData.timestamp || 'N/A'}`;
        }).join("\n");
    
    const link = document.createElement('a');
    link.href = encodeURI(csvContent);
    link.download = `credentials_${taskId}.csv`;
    link.click();
  };

  const exportVulnerabilities = () => {
    if (!taskData?.results?.vulnerabilities) return;
    
    const vulns = taskData.results.vulnerabilities;
    const csvContent = "data:text/csv;charset=utf-8," 
      + "Severity,Description,Target,Timestamp\n"
      + vulns.map(vuln => {
          const severity = vuln.severity || 'MEDIUM';
          const description = (vuln.description || vuln).replace(/,/g, ';');
          return `${severity},"${description}",${taskData.target || 'N/A'},${taskData.timestamp || 'N/A'}`;
        }).join("\n");
    
    const link = document.createElement('a');
    link.href = encodeURI(csvContent);
    link.download = `vulnerabilities_${taskId}.csv`;
    link.click();
  };

  const exportPorts = () => {
    if (!taskData?.results?.detailed_ports) return;
    
    const ports = taskData.results.detailed_ports;
    const csvContent = "data:text/csv;charset=utf-8," 
      + "Port,Protocol,State,Service,Version,Target,Timestamp\n"
      + ports.map(port => {
          return `${port.port},${port.protocol},${port.state},${port.service},"${port.version || ''}",${taskData.target || 'N/A'},${taskData.timestamp || 'N/A'}`;
        }).join("\n");
    
    const link = document.createElement('a');
    link.href = encodeURI(csvContent);
    link.download = `ports_${taskId}.csv`;
    link.click();
  };
  ...
  return (
    <div style={{ 
      ...
      <h4 style={{ margin: '0 0 10px 0', color: '#00ff88', fontSize: '14px' }}>
        📥 Export et Téléchargement - {tool?.toUpperCase()}
      </h4>
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '15px', flexWrap: 'wrap' }}>
            ...
            <button
              ...
              style={{
                padding: '8px 12px',
                background: reportGenerating ? '#666' : 'linear-gradient(45deg, #f59e0b, #d97706)',
                color: 'white',
                border: 'none',
                borderRadius: '6px',
            ...
            >
              {reportGenerating ? '⏳ Génération...' : '🌐 Rapport HTML'}
            </button>

            {/* Boutons spécifiques selon le type de scan */}
            {tool === 'tcpdump' && (
              <>
                <button
                  onClick={downloadPcapFile}
                  style={{
                    padding: '8px 12px',
            ...
            {tool === 'hydra' && taskData?.results?.credentials_found?.length > 0 && (
              <button
            ...
            {tool === 'nikto' && taskData?.results?.vulnerabilities?.length > 0 && (
              <button
              ...
              >
                🚨 Export CSV Vulnérabilités
              </button>
            )}

            {tool === 'nmap' && taskData?.results?.detailed_ports?.length > 0 && (
              <button
                onClick={exportPorts}
                style={{
                  padding: '8px 12px',
          ...
          <div style={{ 
            ...
            <div style={{ color: '#888', fontSize: '10px', display: 'flex', gap: '12px' }}>
              <span>📊 Task: {taskId}</span>
              <span>🎯 Target: {taskData?.target || 'N/A'}</span>
              <span>⏰ {taskData?.timestamp || 'N/A'}</span>
              {taskData?.execution_time && <span>⚡ {taskData.execution_time}</span>}
            </div>
          </div>
          ...
          {showFrames && pcapFrames.length > 0 && (
            <div style={{
              ...
              <div style={{ 
                ...
                <h5 style={{ color: '#00ff88', margin: 0, fontSize: '13px' }}>
...
// ================================
// COMPOSANT RAPPORT HTML
// ================================
const ReportGenerator = ({ taskId, taskData, tool }) => {
  ...
  const generateReport = async () => {
    ...
    try {
      const response = await apiService.request(`/scan/report/${taskId}`, {
        method: 'POST',
      ...
      // Auto-téléchargement
      const link = document.createElement('a');
      link.href = `${getApiBaseUrl()}${response.report_url}`;
      link.download = `report_${taskId}.html`;
      link.target = '_blank';
      document.body.appendChild(link);
      link.click();
  ...
  return (
    <div style={{ display: 'inline-flex', gap: '8px', alignItems: 'center' }}>
      <button
        ...
        style={{
          padding: '6px 12px',
          background: isGenerating ? '#666' : 'linear-gradient(45deg, #f59e0b, #d97706)',
          color: 'white',
          border: 'none',
          borderRadius: '4px',
      ...
      {error && (
        <span style={{ color: '#ff6b6b', fontSize: '10px' }}>❌ {error}</span>
      )}
      
      {reportUrl && (
        <span style={{ color: '#22c55e', fontSize: '10px' }}>✅ Généré</span>
      )}
    </div>
  );
};
...
const TcpdumpTab = () => {
  ...
  interface_,
  captureMode,
  captureMode === 'time' ? parseInt(duration) : undefined,
  captureMode === 'count' ? parseInt(packetCount) : undefined,
  filter || undefined
);
      setCurrentTaskId(response.task_id);
    } catch (error) {
